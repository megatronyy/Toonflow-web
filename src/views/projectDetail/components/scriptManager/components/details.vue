...
function toggleEditTitle() {
  if (!isEditingTitle.value) {
    editableTitle.value = props.item.title;
...
function saveTitle() {
  if (editableTitle.value.trim()) {
    props.item.title = editableTitle.value.trim().substring(0, 10); // 限制长度为10
  }
...
//确认
function onConfirm() {
  console.log("%c Line:91 🍉 editableTitle.value", "background:#ea7e5c", props.item.title);
  console.log("%c Line:106 🍖 props.item.content", "background:#ed9ec7", props.item.content);
  detailsShow.value = false;
}
